...
void PlotEVUncerts_ppfxcomp() {
  ...
  std::string filename_ppfx = "FluxErrors_UncertPoints_Total_onaxis.root";
  std::string filename = "FluxErrors_UncertPoints_Total_onaxis_allpca.root";
  ...
  std::map<int, std::map<beam, std::map<spec, TH1 *>>> Hists_ppfx;

  TFile f(filename.c_str());
  TFile f_ppfx(filename_ppfx.c_str());
  for (int i = 0; i < 6; ++i) {
    for (auto bf : beamnames) {
      for (auto s : specnames) {
        ...
        TH1F *hist_ppfx = nullptr;
        f_ppfx.GetObject(pname.c_str(), hist_ppfx);
        if (!hist) {
          std::cout << "[ERROR] Failed to read: " << pname << std::endl;
          abort();
        }
        Hists_ppfx[i][bf.first][s.first] = hist_ppfx;
      }
    }
  }
  ...
  for (auto s : specnames) {
    for (auto bf : beamnames) {
      ...
      for (int i = 0; i < 6; ++i) {
        ...
        TH1 *h_ppfx = Hists_ppfx[i][bf.first][s.first];
        ...
        h_ppfx->SetLineColor(Colors[i]);
        h_ppfx->SetLineWidth(2);
        h_ppfx->SetLineStyle(2);
  ...
  c1.SaveAs("tikz/EvUncerts_allpca_ppfxcomp.tex");
}
