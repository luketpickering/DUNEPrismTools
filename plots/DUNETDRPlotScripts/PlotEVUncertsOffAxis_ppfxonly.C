...
void PlotEVUncertsOffAxis_ppfxonly() {
  ...
  std::string filename = "FluxErrors_UncertPoints_Total.root";
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    if (i == 0) {
      c1.SaveAs("EvUncerts_offaxis_total_ppfxonly.pdf");
    } else {
      c1.SaveAs(
          ("EvUncerts_offaxis_component_" + std::to_string(i - 1) + "_ppfxonly.pdf")
              .c_str());
    }
  }
}
