...
int main(int argc, char const *argv[]) {
  ...
  if (!IsGauss) {
    ...
    NDOverFDFitScaleFactor = OnAxisPeak / TargetMax;
  ...
  std::cout << "Used " << Fluxes.size() << " fluxes to fit "
            << (binHigh - binLow) << " bins. (Chi2 = " << Chi2_last
  ...
  TTree *OscConfigTree_output = new TTree("OscConfigTree", "");

  Int_t NFluxes = Fluxes.size();
  Int_t NIterations = NCalls;
  Double_t Chi2 = Chi2_last;
  Double_t Reg_contrib = reg_last;
  OscConfigTree_output->Branch("NFluxes", &NFluxes, "NFluxes/I");
  OscConfigTree_output->Branch("NIterations", &NIterations, "NIterations/I");
  OscConfigTree_output->Branch("Chi2", &Chi2, "Chi2/D");
  OscConfigTree_output->Branch("Reg_contrib", &Reg_contrib, "Reg_contrib/D");

  OscConfigTree_output->Branch("IsGaussFit", &IsGauss, "IsGaussFit/O");

  TH1D *TargetFlux_orignorm =
      static_cast<TH1D *>(TargetFlux->Clone("TargetFlux_orignorm"));
  TH1D *OscFlux_orignorm =
      static_cast<TH1D *>(OscFlux->Clone("OscFlux_orignorm"));
  TargetFlux_orignorm->Scale(1.0 / NDOverFDFitScaleFactor);
  OscFlux_orignorm->Scale(1.0 / NDOverFDFitScaleFactor);

  NDOverFDFitScaleFactor =
      (SummedFlux->GetMaximum() / TargetFlux_orignorm->GetMaximum());

  if (!IsGauss) {
    TTree *OscConfigTree_inp = OpenTChainWithFileList("OscConfigTree", inpFile);
    if (OscConfigTree_inp) {
      Double_t DipAngle;
      Double_t OscParams[6];
      OscConfigTree_inp->SetBranchAddress("DipAngle", &DipAngle);
      OscConfigTree_inp->SetBranchAddress("OscParams", &OscParams);
      OscConfigTree_inp->GetEntry(0);

      OscConfigTree_output->Branch("DipAngle", &DipAngle, "DipAngle/D");
      OscConfigTree_output->Branch("OscParams", &OscParams, "OscParams[6]/D");
    } else {
      std::cout << "[WARN]: Oscillation parameter config tree not found in "
                   "input file: "
                << inpFile << ", cannot pass through." << std::endl;
    }

    Double_t OOR_contrib = OOR_last;
    Double_t FitRange[2] = {FitBetween_low, FitBetween_high};

    OscConfigTree_output->Branch("OOR_contrib", &OOR_contrib, "OOR_contrib/D");
    OscConfigTree_output->Branch("FitRange", &FitRange, "FitRange[2]/D");
    OscConfigTree_output->Branch("NDOverFDFitScaleFactor",
                                 &NDOverFDFitScaleFactor,
                                 "NDOverFDFitScaleFactor/D");
  } else {
    Double_t GaussC_conf = GaussC;
    Double_t GaussW_conf = GaussW;

    OscConfigTree_output->Branch("GaussC_conf", &GaussC_conf, "GaussC_conf/D");
    OscConfigTree_output->Branch("GaussW_conf", &GaussW_conf, "GaussW_conf/D");
  }
  OscConfigTree_output->Fill();
