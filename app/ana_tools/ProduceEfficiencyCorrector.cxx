...
int main(int argc, char const *argv[]) {
  ...
  std::vector<BoundingBox> StopActiveRegions =
      csRdr.GetStopBoundingBoxes(false);
  ...
  for (BoundingBox const &bb : StopActiveRegions) {
    ...
    MaxToWall = std::max(
        MaxToWall, ((TVector3(bb.Max.data()) - TVector3(bb.Min.data())).Mag()));
  ...
  TH2D *MuonKinematics_all = new TH2D(
      "MuonKinematics_all", ";#it{E}_{#mu};ToWall_{#mu} (m);Count",
      (ERecBinning.size() - 1), ERecBinning.data(), 60, 0, MaxToWall * 1E-2);
  TH2D *MuonKinematics_musel = new TH2D(
      "MuonKinematics_musel", ";#it{E}_{#mu};ToWall_{#mu} (m);Count",
      (ERecBinning.size() - 1), ERecBinning.data(), 60, 0, MaxToWall * 1E-2);
  TH2D *MuonKinematics_seleff = new TH2D(
      "MuonKinematics_eff", ";#it{E}_{#mu};ToWall_{#mu} (m);#epsilon",
      (ERecBinning.size() - 1), ERecBinning.data(), 60, 0, MaxToWall * 1E-2);
  ...
  TH2D *Ehadr_FV_detpos_all = new TH2D(
      ...
      (MaxStopWidth / 2.0));
  TH2D *Ehadr_FV_detpos_hadrsel = new TH2D(
      "Ehadr_FV_detpos_hadrsel",
      ";#it{E}_{Hadr};Detector X position (cm);Count", (ERecBinning.size() - 1),
      ERecBinning.data(), 100, -(MaxStopWidth / 2.0), (MaxStopWidth / 2.0));
  ...
  TH2D *EVisHadr_FV_detpos_all =
      new TH2D("EVisHadr_FV_detpos_all",
  ...
  TH2D *ETrueNonNeutron_FV_detpos_all =
      new TH2D("ETrueNonNeutron_FV_detpos_all",
               ";#it{E}_{Hadr, NonNeutron};Detector X position (cm);Count",
               (ERecBinning.size() - 1), ERecBinning.data(), 100,
               -(MaxStopWidth / 2.0), (MaxStopWidth / 2.0));
  ...
  TH2D *ETrueNonNeutron_FV_detpos_seleff =
      new TH2D("ETrueNonNeutron_FV_detpos_seleff",
               ";#it{E}_{Hadr, NonNeutron};Detector X position (cm);#epsilon",
               (ERecBinning.size() - 1), ERecBinning.data(), 100,
               -(MaxStopWidth / 2.0), (MaxStopWidth / 2.0));
  ...
  TH2D *Ehadr_FV_abspos_hadrsel = new TH2D(
      "Ehadr_FV_abspos_hadrsel",
      ";#it{E}_{Hadr};Detector X position (cm);Count", (ERecBinning.size() - 1),
      ERecBinning.data(), 100, MinAbsX, MaxAbsX);
  TH2D *Ehadr_FV_abspos_seleff = new TH2D(
      "Ehadr_FV_abspos_seleff",
      ";#it{E}_{Hadr};Detector X position (cm);#epsilon",
      (ERecBinning.size() - 1), ERecBinning.data(), 100, MinAbsX, MaxAbsX);

  TH2D *ETrueNonNeutron_FV_abspos_all = new TH2D(
      "ETrueNonNeutron_FV_abspos_all",
      ";#it{E}_{Hadr, NonNeutron};Detector X position (cm);Count",
      (ERecBinning.size() - 1), ERecBinning.data(), 100, MinAbsX, MaxAbsX);
  TH2D *ETrueNonNeutron_FV_abspos_hadrsel = new TH2D(
      "ETrueNonNeutron_FV_abspos_hadrsel",
      ";#it{E}_{Hadr, NonNeutron};Detector X position (cm);Count",
      (ERecBinning.size() - 1), ERecBinning.data(), 100, MinAbsX, MaxAbsX);
  TH2D *ETrueNonNeutron_FV_abspos_seleff = new TH2D(
      "ETrueNonNeutron_FV_abspos_seleff",
      ";#it{E}_{Hadr, NonNeutron};Detector X position (cm);#epsilon",
      (ERecBinning.size() - 1), ERecBinning.data(), 100, MinAbsX, MaxAbsX);
  ...
  for (Long64_t e_it = 0; e_it < NEntries; ++e_it) {
    ...
    // Don't want to include events outside acceptance in efficiency correction.
    bool HadrAccepted =
        ((HadrVisAcceptance == 0) ||
    ...
    double ToWall =
        CalculateToWall(StopActiveRegions[edr.stop], TrStr, TrDir) * 1E-2;

    MuonKinematics_all->Fill(edr.GetProjection(DepositsSummary::kEFSLep_True),
                             ToWall, edr.stop_weight * ProtonFakeDataWeight);
    MuonKinematics_musel->Fill(edr.GetProjection(DepositsSummary::kEFSLep_True),
                               ToWall,
                               edr.stop_weight * ProtonFakeDataWeight *
                                   double(edr.LepExitKE > SelMuExitKE));
    ...
    Ehadr_FV_detpos_hadrsel->Fill(
        ...
        edr.stop_weight * ProtonFakeDataWeight *
            double(edr.TotalNonlep_Dep_veto < HadrVeto));

    EVisHadr_FV_detpos_all->Fill(edr.GetProjection(DepositsSummary::kEHadr_vis),
                                 edr.vtxInDetX,
                                 edr.stop_weight * ProtonFakeDataWeight);
    ...
    Ehadr_FV_abspos_all->Fill(edr.GetProjection(DepositsSummary::kEHadr_True),
                              edr.vtx[0],
                              edr.stop_weight * ProtonFakeDataWeight);
    Ehadr_FV_abspos_hadrsel->Fill(
        edr.GetProjection(DepositsSummary::kEHadr_True), edr.vtx[0],
        edr.stop_weight * ProtonFakeDataWeight *
            double(edr.TotalNonlep_Dep_veto < HadrVeto));

    ETrueNonNeutron_FV_detpos_all->Fill(
        edr.GetProjection(DepositsSummary::kENonNeutronHadr_True),
        edr.vtxInDetX, edr.stop_weight * ProtonFakeDataWeight);
    ETrueNonNeutron_FV_detpos_hadrsel->Fill(
        edr.GetProjection(DepositsSummary::kENonNeutronHadr_True),
        edr.vtxInDetX,
        edr.stop_weight * ProtonFakeDataWeight *
            double(edr.TotalNonlep_Dep_veto < HadrVeto));

    ETrueNonNeutron_FV_abspos_all->Fill(
        edr.GetProjection(DepositsSummary::kENonNeutronHadr_True), edr.vtx[0],
        edr.stop_weight * ProtonFakeDataWeight);
    ETrueNonNeutron_FV_abspos_hadrsel->Fill(
        edr.GetProjection(DepositsSummary::kENonNeutronHadr_True), edr.vtx[0],
        edr.stop_weight * ProtonFakeDataWeight *
            double(edr.TotalNonlep_Dep_veto < HadrVeto));
  }
  ...
  ETrueNonNeutron_FV_detpos_seleff->Divide(ETrueNonNeutron_FV_detpos_hadrsel,
                                           ETrueNonNeutron_FV_detpos_all);
  ETrueNonNeutron_FV_abspos_seleff->Divide(ETrueNonNeutron_FV_abspos_hadrsel,
                                           ETrueNonNeutron_FV_abspos_all);
  of->Write();
  of->Close();
}
