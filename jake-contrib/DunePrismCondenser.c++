...
int main(int argc, char* argv[]) {
  ...
  DunePrismCondenser dpc = DunePrismCondenser(inFileName, outFileName,
                                              codeFileName, fullDet, nEntries);
  dpc.Condense();
  dpc.Finalize();
  return 0;
}

DunePrismCondenser::DunePrismCondenser(std::string inFileName,
                                       std::string outFileName,
                                       std::string codeFileName,
                                       DetectorStop* fullDet, int N) {
#ifdef DEBUG
  std::cout << inFileName << std::endl;
  std::cout << outFileName << std::endl;
  std::cout << codeFileName << std::endl;
#endif

  fin = new TFile(inFileName.c_str(), "READ");
  inTree = (TTree*)fin->Get("argon");

  fcode = new TFile(codeFileName.c_str(), "READ");
  codeTree = (TTree*)fcode->Get("gRooTracker");

  if (!inTree) std::cout << "Can't get argon tree" << std::endl;
#ifdef DEBUG
  else
    std::cout << "Got argon tree" << std::endl;
#endif

  nEntries = N;
  if (nEntries > inTree->GetEntries() || nEntries == -1)
  ...
  if (fullDet != NULL) {
    fullDetTree =
        new TTree("fullDetTree", "Positional deposits over full detector");
    InitDetector();
  }

  SetInBranches();
  std::cout << "Set In branches" << std::endl;
}
...
void DunePrismCondenser::Condense() {
  for (int ie = 0; ie < nEntries; ++ie) {
    ...
    vtx_Z_full = EvtVtx[2];
    eventNumFull = ev;
    eventCode = (TObjString*)code->Clone();
    ...
    if (vtx_X_full < (fullDet->shift - fullDet->detectorSizeX * .5) * 100. ||
      ...
      std::cout << "Skipping " << ie << std::endl
                << "\t" << vtx_X_full << " " << vtx_Y_full << " " << vtx_Z_full
                << std::endl
                << "\t" << (fullDet->shift - fullDet->detectorSizeX * .5) * 100.
                << " " << (fullDet->shift + fullDet->detectorSizeX * .5) * 100.
                << " " << fullDet->detectorSizeY * .5 * 100. << " "
                << fullDet->detectorSizeZ * .5 * 100. << std::endl;
      continue;
    }
    ...
    for (int ip = 0; ip < ni; ++ip) {
      ...
      chain[ip + 1] = 0;
      if ((abs(PIDi[ip]) == 13 || abs(PIDi[ip]) == 11 || PIDi[ip] == nuPID) &&
          nLepFull < 1) {
        ...
        q->SetPxPyPzE(pxa - pxi[ip], pya - pyi[ip], pza - pzi[ip],
                      EnuFull - eLepTrueFull);

        Q2TrueFull =
            fabs(pow((ekina - eLepTrueFull), 2) - pow((pxi[ip] - pxa), 2) -
                 pow((pyi[ip] - pya), 2) - pow((pzi[ip] - pza), 2));

        yTrueFull = 1 - eLepTrueFull / ekina;
        double mN = .93827208;
        W_rest_full =
            sqrt(-Q2TrueFull + 2 * mN * (ekina - eLepTrueFull) + mN * mN);

        nLepFull++;

        if (abs(PIDi[ip]) == abs(nuPID) - 1)
          flagCCFull = 1;
        else if (PIDi[ip] == nuPID)
          flagCCFull = 0;
        ...
        DepoHadron* hadron = new DepoHadron(
            PIDi[ip], (ip + 1), ekini[ip] + mi[ip], wallX, wallY, wallZ);
        ...
        nProtonFull++;
        DepoHadron* hadron =
            new DepoHadron(PIDi[ip], (ip + 1), ekini[ip], wallX, wallY, wallZ);
        FSHadrons[ip + 1] = hadron;
        ...
        //            std::cout << "Found Other! " << PIDi[ip] <<  " " << ip + 1
        //            << std::endl;
        nOther++;
      }
    }
    ...
    for (size_t it = 0; it < xBins.size(); ++it) {
      for (size_t jt = 0; jt < yBins.size(); ++jt) {
        for (size_t kt = 0; kt < zBins.size(); ++kt) {
    ...
    for (int i = 0; i < nstep; ++i) {
      ...
      if (chain[track[i]] == 0) {                    // Primary
        if (PID[i] == lepPDGFull && !flagLepExit) {  // lepton
          ...
          if (abs(PID[i]) == 11) {
            // Place in any bin in x, but check that it's within
            // the fiducial range in y,z
            if (xe[i] >= xBins.at(0) && xe[i] <= xBins.at(xBins.size() - 1) &&
                ye[i] >= yBins.at(1) && ye[i] <= yBins.at(2) &&
                ze[i] >= zBins.at(1) && ze[i] <= zBins.at(2)) {
          ...
          if (ye[i] >= yBins.at(0) && ye[i] <= yBins.at(3) &&
            ...
            int bins[3] = {GetBinX(xe[i]), GetBinY(ye[i]), GetBinZ(ze[i])};
          ...
          if (ye[i] <= yBins.at(0) || ye[i] >= yBins.at(3)) {  // Muon exitting
                                                               // y
            flagLepExitY = true;
            flagLepExit = true;
          }
            ...
            if (xe[i] >= xBins.at(0) && xe[i] <= xBins.at(xBins.size() - 1) &&
              ...
              switch (abs(PID[i])) {
                ...
                case 2112:
                  ...
                  neutronPrimaryTimeDep[bins[0]][bins[1]][bins[2]] +=
                      edep[i] * te[i];
                  break;
                case 211:
        ...
        if (itChain == 1) {  // Lepton
          ...
          if (abs(PIDi[0]) == 11) {
            ...
            //< Within X AND < outside in y OR outside in z > >
            else if ((xe[i] >= xBins.at(0) &&
                      xe[i] <= xBins.at(xBins.size() - 1)) &&
                     (((ye[i] >= yBins.at(0) && ye[i] < yBins.at(1)) ||
                       (ye[i] > yBins.at(2) && ye[i] <= yBins.at(3))) ||
                      ((ze[i] >= zBins.at(0) && ze[i] < zBins.at(1)) ||
                       (ze[i] > zBins.at(2) && ze[i] <= zBins.at(3))))) {
              eElectronShowerDepOutside += edep[i];
            }
          }
            ...
            if (xe[i] >= xBins.at(0) && xe[i] <= xBins.at(xBins.size() - 1) &&
              ...
              switch (abs(FSHadrons[itChain]->PDG)) {
                ...
                case 2112:
                  ...
                  neutronSecondaryTimeDep[bins[0]][bins[1]][bins[2]] +=
                      edep[i] * te[i];
                  break;
                case 211:
      ...
      for (size_t jt = 0; jt < yBins.size() - 1; ++jt) {
        for (size_t kt = 0; kt < zBins.size() - 1; ++kt) {
          if (eNeutronPrimaryDep[it][jt][kt] < 0.000000001) {
            if (!(neutronPrimaryTimeDep[it][jt][kt] < 0.0000000001)) {
              std::cout << "Error: different dep\n";
            }
            continue;
          }
          neutronPrimaryTimeDep[it][jt][kt] =
              neutronPrimaryTimeDep[it][jt][kt] /
              eNeutronPrimaryDep[it][jt][kt];
          if (eNeutronSecondaryDep[it][jt][kt] < 0.000000001) {
            if (!(neutronSecondaryTimeDep[it][jt][kt] < 0.0000000001)) {
              std::cout << "Error: different dep\n";
            }
            continue;
          }
          neutronSecondaryTimeDep[it][jt][kt] =
              neutronSecondaryTimeDep[it][jt][kt] /
              eNeutronSecondaryDep[it][jt][kt];
        }
      }
    }
...
void DunePrismCondenser::SetInBranches() {
  ...
  codeTree->SetBranchAddress("EvtCode", &code);
}
...
void DunePrismCondenser::InitDetector() {
  ...
  for (size_t i = 0; i < xBins.size(); ++i) {
    for (size_t j = 0; j < yBins.size(); ++j) {
      for (size_t k = 0; k < zBins.size(); ++k) {
  ...
  fullDetTree->Branch("eLepPrimaryDepFull", &eLepPrimaryDepFull,
                      "eLepPrimaryDep[400][3][3]/D");
  fullDetTree->Branch("eHadPrimaryDep", &eHadPrimaryDep,
                      "eHadPrimaryDep[400][3][3]/D");
  fullDetTree->Branch("eProtonPrimaryDep", &eProtonPrimaryDep,
                      "eProtonPrimaryDep[400][3][3]/D");
  fullDetTree->Branch("eNeutronPrimaryDep", &eNeutronPrimaryDep,
                      "eNeutronPrimaryDep[400][3][3]/D");
  fullDetTree->Branch("ePiCPrimaryDep", &ePiCPrimaryDep,
                      "ePiCPrimaryDep[400][3][3]/D");
  fullDetTree->Branch("ePi0PrimaryDep", &ePi0PrimaryDep,
                      "ePi0PrimaryDep[400][3][3]/D");
  fullDetTree->Branch("eOtherPrimaryDep", &eOtherPrimaryDep,
                      "eOtherPrimaryDep[400][3][3]/D");

  fullDetTree->Branch("eLepSecondaryDepFull", &eLepSecondaryDepFull,
                      "eLepSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("eHadSecondaryDep", &eHadSecondaryDep,
                      "eHadSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("eProtonSecondaryDep", &eProtonSecondaryDep,
                      "eProtonSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("eNeutronSecondaryDep", &eNeutronSecondaryDep,
                      "eNeutronSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("ePiCSecondaryDep", &ePiCSecondaryDep,
                      "ePiCSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("ePi0SecondaryDep", &ePi0SecondaryDep,
                      "ePi0SecondaryDep[400][3][3]/D");
  fullDetTree->Branch("eOtherSecondaryDep", &eOtherSecondaryDep,
                      "eOtherSecondaryDep[400][3][3]/D");
  fullDetTree->Branch("neutronPrimaryTimeDep", &neutronPrimaryTimeDep,
                      "neutronPrimaryTimeDep[400][3][3]/D");
  fullDetTree->Branch("neutronSecondaryTimeDep", &neutronSecondaryTimeDep,
                      "neutronSecondaryTimeDep[400][3][3]/D");

  fullDetTree->Branch("eElectronShowerDepInside", &eElectronShowerDepInside,
                      "eElectronShowerDepInside[400]/D");
  fullDetTree->Branch("eElectronShowerDepOutside", &eElectronShowerDepOutside,
                      "eElectronShowerDepOutside/D");
  std::cout << "Made Branches" << std::endl;
}
...
int DunePrismCondenser::GetBinY(double y) {
  ...
  if (y <= yBins.at(1)) {
    ybin = 0;
  } else if (y <= yBins.at(2)) {
    ybin = 1;
  } else if (y <= yBins.at(3)) {
    ybin = 2;
  }
  return ybin;
}
...
int DunePrismCondenser::GetBinZ(double z) {
  ...
  if (z <= zBins.at(1)) {
    zbin = 0;
  } else if (z <= zBins.at(2)) {
    zbin = 1;
  } else if (z <= zBins.at(3)) {
    zbin = 2;
  }
  return zbin;
}
