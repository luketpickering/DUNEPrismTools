...
if [ ! -d "$1" ]; then
  mkdir /home/calcuttj/DUNEPrismSim/FHC_FD/$1
  echo "Made directory /home/calcuttj/DUNEPrismSim/FHC_FD/$1/"
  mkdir FHC_FD/$1
  echo "Made directory FHC_FD/$1/"
fi
 
cat FHC_FD_input.list | while read line 
do
  echo $line
  string=$line
  IFS='.'
  ...
  OUTFILE=FHC_FD/$1/dpa.FHC_FD.${array[2]}.${array[3]}.pbs

  cat $FILE > $OUTFILE  
  echo \#PBS -l file=20gb >> ${OUTFILE}
  echo \#PBS -N ${run}_analysis >> ${OUTFILE} 
  echo DATAPATH=/mnt/research/NuInt/Dune_Flux/OptimizedEngineeredSept2017Review/gevgen_fnal/rootracker/FHC_FD >> ${OUTFILE}
