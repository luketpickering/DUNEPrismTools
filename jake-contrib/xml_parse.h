...
inline std::vector<DetectorStop> ReadDetectorStopConfig(
  ...
  while (root_child) {
    if (rptagname == xE.GetNodeName(root_child)) {
      bool found_rp;
      std::string name =
          GetXMLAttributeValue<std::string>(root_child, "Name", found_rp);
      ...
      while (rp_child) {
        if (dstagname == xE.GetNodeName(rp_child)) {
          detDefinition.detectorSizeX =
              GetXMLAttributeValue<double>(rp_child, "detectorSizeX", found[0]);
          detDefinition.detectorSizeY =
              GetXMLAttributeValue<double>(rp_child, "detectorSizeY", found[1]);
          detDefinition.detectorSizeZ =
              GetXMLAttributeValue<double>(rp_child, "detectorSizeZ", found[2]);

          detDefinition.fiducialGapX =
              GetXMLAttributeValue<double>(rp_child, "fiducialGapX", found[3]);
          detDefinition.fiducialGapY =
              GetXMLAttributeValue<double>(rp_child, "fiducialGapY", found[4]);
          detDefinition.fiducialGapZ =
              GetXMLAttributeValue<double>(rp_child, "fiducialGapZ", found[5]);

          detDefinition.shift =
              GetXMLAttributeValue<double>(rp_child, "shift", found[6]);
...
inline DetectorStop *GetFullDetectorConfig(std::string const &fname,
                                           std::string const &RPName = "") {
  TXMLEngine xE;
  xE.SetSkipComments(true);
  XMLDocPointer_t doc = xE.ParseFile(fname.c_str());
