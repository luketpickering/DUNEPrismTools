...
BoundingBox::BoundingBox() : Max({0, 0, 0}), Min({0, 0, 0}) {}
BoundingBox::BoundingBox(std::array<double,3> const &Max,
  std::array<double,3> const &Min) {
  this->Max[0] = Max[0];
  this->Max[1] = Max[1];
  this->Max[2] = Max[2];
...
bool BoundingBox::Contains(std::array<double,3> const &pos) const {
  bool gt = true;
...
std::string BoundingBox::Print() const {
  std::stringstream ss("");
...
void BoundingBox::Sort() {
  std::array<double, 3> nMax;
  std::array<double, 3> nMin;
...
void BoundingBox::CalcPlanes() {
  PlaneNormals[0] = std::array<double, 3>{-1, 0, 0};
  PlaneNormals[1] = std::array<double, 3>{1, 0, 0};

  PlaneNormals[2] = std::array<double, 3>{0, -1, 0};
  PlaneNormals[3] = std::array<double, 3>{0, 1, 0};

  PlaneNormals[4] = std::array<double, 3>{0, 0, -1};
  PlaneNormals[5] = std::array<double, 3>{0, 0, 1};

  std::array<double, 3> Halfs{(Max[0] - Min[0]) / 2.0f,
                               (Max[1] - Min[1]) / 2.0f,
                               (Max[2] - Min[2]) / 2.0f};

  PlaneCenters[0] = std::array<double, 3>{Min[0], Halfs[1], Halfs[2]};
  PlaneCenters[1] = std::array<double, 3>{Max[0], Halfs[1], Halfs[2]};

  PlaneCenters[2] = std::array<double, 3>{Halfs[0], Min[1], Halfs[2]};
  PlaneCenters[3] = std::array<double, 3>{Halfs[0], Max[1], Halfs[2]};

  PlaneCenters[4] = std::array<double, 3>{Halfs[0], Halfs[1], Min[2]};
  PlaneCenters[5] = std::array<double, 3>{Halfs[0], Halfs[1], Max[2]};
  ...
  // to
  // test against.
  PlaneBBs[0] = std::array<double, 4>{Min[2], Min[1], Max[2], Max[1]};
  PlaneBBs[1] = std::array<double, 4>{Min[0], Min[2], Max[0], Max[2]};
  PlaneBBs[2] = std::array<double, 4>{Min[1], Min[0], Max[1], Max[0]};
}
...
double CalculateToWall(BoundingBox const &BB, TVector3 const &TrStart,
  ...
  if (!BB.Contains({TrStart[0],TrStart[1],TrStart[2]})) {
    std::cout
