...
std::pair<char const *, char const *> SSAX::HandleAttribute(std::ifstream &ifs) {
...
void SSAX::HandleCommentTag(std::ifstream &ifs) { IgnoreAllToRightBracket(ifs); }
...
GENIESplineGetter::GENIESplineGetter(std::vector<std::string> sn) : SSAX() {
  ...
  GENIEXSecReader::GENIEXSecReader(std::string const &XSecInputsFile) : saxParser(nullptr) {
      TXMLEngine xE;
    ...
    void GENIEXSecReader::Read(std::string const &gxmlfile) { saxParser->ParseFile(gxmlfile.c_str()); }

    std::vector<std::pair<std::string, TGraph>> GENIEXSecReader::GetTGraphs(double min,
                                                           double max) {
